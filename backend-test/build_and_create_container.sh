...
RUN pip3 install -r requirements.txt"> $WORKDIR/Dockerfile

else
	CURRENT_VERSION=$(docker image ls | grep $APP_NAME | awk '{print $2}' | sort | tail -1)
	NEW_VERSION=$((CURRENT_VERSION+1))
	sed -i "s/python\:alpine/$APP_NAME:$CURRENT_VERSION/g" $WORKDIR/Dockerfile
fi
...
if [[ $OUTPUT =~ "Successfully" ]]; then
	...
	$DOCKER_DAEMON rm -f $APP_NAME > /dev/null 2>&1
 	CONTAINER_ID=$($DOCKER_DAEMON run -d -p $LOCAL_PORT:$REMOTE_PORT --name $APP_NAME $APP_NAME:$NEW_VERSION python3 /usr/src/app.py)
